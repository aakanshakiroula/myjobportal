...
const JobDescription = () => {
  ...
  const applyJobHandler = async () => {
    try {
      ...
      const res = await axios.get(`https://myjobportal-2.onrender.com/api/v1/application/apply/${params.id}`);
      if (res.data.success) {
  ...
  useEffect(() => {
    const fetchSingleJob = async () => {
      try {
        axios.defaults.withCredentials = true;
        const res = await axios.get(`https://myjobportal-2.onrender.com/api/v1/job/${params.id}`);
        if (res.data.success) {
